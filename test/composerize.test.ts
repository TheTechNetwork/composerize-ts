...
const loadYaml = (file: string): unknown => yamlParse(readFileSync(file, 'utf8'));
const dumpYaml = (obj: unknown): string => yamlStringify(obj, { indent: 4, lineWidth: 0 });
...
test('parses simple docker run command', async () => {
  ...
  const expected = loadYaml(path.join(__dirname, './data/basic.yaml'));
  expect(result.yaml).toEqual(dumpYaml(expected));
});
...
test('parses simple docker run command and generates docker-compose.yml with version info', async () => {
  ...
  const expected = loadYaml(path.join(__dirname, './data/basic-with-version.yaml'));
  expect(result.yaml).toEqual(dumpYaml(expected));
});
...
test('parses simple docker run command with custom command', async () => {
  ...
  const result = composerize(cmd);
  const expected = loadYaml(path.join(__dirname, './data/custom-command.yaml'));
  expect(result.yaml).toEqual(dumpYaml(expected));
});
...
test('can publish ports', async () => {
  ...
  const result = composerize(cmd);
  const expected = loadYaml(path.join(__dirname, './data/ports.yaml'));
  expect(result.yaml).toEqual(dumpYaml(expected));
});
...
test('test a complex command', async () => {
  ...
  const result = composerize(cmd);
  const expected = loadYaml(path.join(__dirname, './data/complex.yaml'));
  expect(result.yaml).toEqual(dumpYaml(expected));
...
test('test an even more complex command', async () => {
  ...
  const result = composerize(cmd);
  const expected = loadYaml(path.join(__dirname, './data/even-more-complex.yaml'));
  expect(result.yaml).toEqual(dumpYaml(expected));
  expect(result.messages.length).toEqual(0);
});
